...
export const AISettings: React.FC = () => {
    ...
    const [latency, setLatency] = useState<number | null>(142);
    ...
    useEffect(() => {
        const loadConfig = async () => {
            try {
                const data = await getCollectionConfig<any>('system', 'ai');
    ...
    const handleSave = async () => {
        try {
            await updateCollectionConfig('system', 'ai', aiConfig);
            toast.success('AI Engine yapılandırması güncellendi!');
        } catch (err) {
            toast.error('Girdiğiniz bilgiler kaydedilemedi.');
        }
    };
    ...
    const handleLatencyTest = () => {
        ...
        setTimeout(() => {
            ...
            toast.success('Gecikme testi tamamlandı!');
        }, 1500);
    };
    ...
    return (
        <div className="p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-700">
            <header className="flex justify-between items-end">
                <div>
                    <h1 className="text-4xl font-black text-white tracking-tight uppercase">Yapay Zeka Ayarları</h1>
                    <p className="text-white/30 text-sm font-medium">LLM modellerini ve zeka parametrelerini yapılandırın.</p>
                </div>
                <button
                    onClick={handleSave}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="md:col-span-2 glass p-8 rounded-[32px] border-white/5 space-y-8">
                    <div className="flex items-center gap-3 text-gold">
                        <Cpu size={20} />
                        <h2 className="text-sm font-black uppercase tracking-widest">Model Yapılandırması</h2>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-1">Birincil Model</label>
                            <select
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-white/40 ml-1">API Anahtarı</label>
                    ...
                    <div className="space-y-6 pt-4 border-t border-white/5">
                        <div className="space-y-4">
                            <div className="flex justify-between">
                                <label className="text-[10px] font-black uppercase tracking-widest text-white/40">Sıcaklık (Temperature) ({aiConfig.temperature})</label>
                                <span className="text-[10px] text-white/20 font-bold uppercase italic">{aiConfig.temperature > 0.8 ? 'Yaratıcı' : 'Kesin'}</span>
                            </div>
                            <input
                ...
                <div className="space-y-8">
                    <div className="glass p-8 rounded-[32px] border-white/5 space-y-4">
                        ...
                        <h3 className="text-xl font-black text-white uppercase tracking-tight">AI Tanılama</h3>
                        <p className="text-xs text-white/40 font-medium leading-relaxed">
                            Sistem optimize çalışıyor. Gecikme: <span className="text-gold">{latency ? `${latency}ms` : 'Test ediliyor...'}</span>
                        </p>
                        <button
                            onClick={handleLatencyTest}
                            disabled={latency === null}
                            className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-white text-[10px] font-black uppercase tracking-widest hover:bg-gold/10 hover:border-gold/20 hover:text-gold transition-all disabled:opacity-50"
                        >
                            Gecikme Testini Çalıştır
                        </button>
                    </div>
                    ...
                    <div className="glass p-8 rounded-[32px] border-emerald-500/10 bg-emerald-500/[0.02] space-y-3">
                        <div className="flex items-center gap-2 text-emerald-400">
                            <Zap size={16} />
                            <h4 className="text-[10px] font-black uppercase tracking-widest">Zeka Seviyesi</h4>
                        </div>
                        <p className="text-xs text-white/40 font-medium">Tahmini bakım doğruluğu şu anda <span className="text-emerald-400 font-bold">%94.8</span> seviyesinde.</p>
                    </div>
                </div>
            </div>
        </div>
