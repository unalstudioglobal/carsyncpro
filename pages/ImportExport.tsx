...
export const ScanImport: React.FC = () => {
    ...
    const [status, setStatus]         = useState<'idle' | 'requesting' | 'scanning' | 'found' | 'error' | 'manual'>('idle');
    ...
    const startCamera = async () => {
        ...
        try {
            ...
            if (err.name === 'NotAllowedError') {
                ...
                setErrorMsg(t('importExport.err_cam_fail') + err.message);
            }
            setStatus('error');
        }
    };
    ...
    const scanLoop = () => {
        rafRef.current = requestAnimationFrame(() => {
            ...
            // Önce modern BarcodeDetector API'yi dene
    ...
    const tryJsQR = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, retry: () => void) => {
        if (!jsQRRef.current) { retry(); return; }
    ...
    const handleFound = (code: string) => {
        stopCamera();
        // CarSync transfer kodu formatı: TR-XXX-XXX veya URL içinde kod
        const match = code.match(/TR-[A-Z0-9]{3}-[A-Z0-9]{3}/i);
        const extracted = match ? match[0].toUpperCase() : code.toUpperCase();
    ...
    const handleImport = async (code: string) => {
        ...
        setImporting(true);
        // Simüle edilmiş sunucu doğrulaması — gerçek uygulamada API çağrısı yapılmalı
        setTimeout(() => {
            setImporting(false);
    ...
    return (
        <div className="h-screen bg-black flex flex-col relative overflow-hidden">
            ...
            <header className="absolute top-0 left-0 right-0 z-20 p-5 flex justify-between items-center bg-gradient-to-b from-black/80 to-transparent">
                <button onClick={() => { stopCamera(); navigate(-1); }} className="p-2 -m-2 text-white">
                    <ChevronLeft size={32} />
                </button>
                {status === 'scanning' && (
                    <button onClick={toggleTorch} className={`w-10 h-10 rounded-full flex items-center justify-center transition ${torchOn ? 'bg-yellow-500 text-black' : 'bg-white/10 text-white'}`}>
                        <Zap size={18} />
                    </button>
                )}
            </header>
            ...
            {status === 'requesting' && (
                <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center space-y-4">
                    ...
                    <p className="text-slate-300 text-sm">{t('importExport.req_cam')}</p>
                </div>
            )}
            ...
            {status === 'scanning' && (
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <h2 className="text-white font-bold text-lg mb-2 drop-shadow-lg">{t('importExport.scan_qr')}</h2>
                    <p className="text-slate-300 text-xs mb-8 drop-shadow-md">{t('importExport.scan_hint')}</p>
                    ...
                    <button onClick={() => { stopCamera(); setStatus('manual'); }} className="mt-10 text-xs text-slate-400 hover:text-slate-200 transition underline underline-offset-2">
            ...
            {status === 'found' && (
                <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-8 space-y-6">
                    ...
                    <button onClick={() => { setStatus('scanning'); startCamera(); }} className="text-sm text-slate-400 hover:text-slate-200 underline underline-offset-2 transition">
                ...
                <div className="absolute inset-0 bg-slate-950 flex flex-col">
                    ...
                    <div className="bg-slate-900 p-6 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)] space-y-5">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={() => { startCamera(); }} className="text-xs text-blue-400 hover:text-blue-300 flex items-center space-x-1">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-800 border border-slate-700 rounded-xl p-4 text-center font-mono text-2xl tracking-widest uppercase outline-none focus:border-blue-500 transition-colors"
                            maxLength={10}
                        />
