...
export const Premium: React.FC = () => {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      console.error('Premium aktivasyon hatası:', err);
  ...
  const rows = [
    { label: t('prem.feat1'), free: t('prem.feat1_free'), pro: t('prem.feat1_pro'), icon: Car },
    { label: t('prem.feat2'), free: t('prem.feat2_free'), pro: t('prem.feat1_pro'), icon: FileDown },
    { label: t('prem.feat3'), free: `3${t('prem.per_month')}`, pro: t('prem.feat1_pro'), icon: Sparkles },
    { label: t('prem.feat4'), free: false, pro: true, icon: ArrowRightLeft },
    { label: t('prem.feat5'), free: false, pro: true, icon: FileDown },
    { label: t('prem.feat6'), free: false, pro: true, icon: BarChart2 },
    { label: t('prem.feat7'), free: false, pro: true, icon: Shield },
    { label: t('prem.feat8'), free: false, pro: true, icon: Star },
  ];
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-44 relative z-10 space-y-7">
        ...
        <div className="text-center pt-2 space-y-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1.5 max-w-[260px] mx-auto leading-relaxed">{t('prem.desc')}</p>
          </div>
          <div className="flex items-center justify-center space-x-4 pt-1">
            ...
            <div className="text-left">
              ...
              <p className="text-[11px] text-slate-400 mt-0.5"><span className="text-white font-bold">12.000+</span> {t('prem.trusted_by')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/80 p-1.5 rounded-xl flex border border-white/5 max-w-xs mx-auto">
          ...
          <button onClick={() => setBillingCycle('yearly')} className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all relative ${billingCycle === 'yearly' ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>
            Yıllık
            <span className="absolute -top-3 -right-1 bg-green-500 text-white text-[9px] px-1.5 py-0.5 rounded-full border border-slate-900 font-black">{t('prem.discount', { pct: savingsPct })}</span>
          </button>
        </div>
        ...
        <div className="bg-gradient-to-b from-slate-900/80 to-slate-900/40 border border-amber-500/25 rounded-3xl p-6 text-center relative overflow-hidden">
          ...
          {billingCycle === 'yearly' && (
            <div className="inline-flex items-center space-x-1.5 bg-green-500/15 border border-green-500/30 text-green-400 text-xs font-bold px-3 py-1 rounded-full mb-4">
              <Check size={11} />
              <span>{t('prem.save_yearly', { amount: MONTHLY_PRICE * 12 - YEARLY_PRICE })}</span>
            </div>
          )}
          <div className="flex items-baseline justify-center space-x-1">
            ...
            <span className="text-slate-400 text-sm font-medium self-end mb-1">/{billingCycle === 'yearly' ? 'yıl' : 'ay'}</span>
          </div>
          <p className="text-xs text-amber-400/80 mt-2 font-medium">
            {billingCycle === 'yearly' ? t('prem.yearly_desc', { amt: monthlyEquiv }) : t('prem.monthly_desc')}
          </p>
          <div className="mt-4 pt-4 border-t border-slate-800 flex items-center justify-center space-x-2 text-xs text-slate-400">
            ...
            <span>7 gün içinde memnun kalmazsan <span className="text-green-400 font-bold">tam iade</span></span>
          </div>
        </div>

        {/* Karşılaştırma tablosu */}
        <div>
          <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest text-center mb-4">{t('prem.compare_title')}</h2>
          <div className="bg-slate-900/60 rounded-2xl border border-slate-800 overflow-hidden">
            <div className="grid grid-cols-3 border-b border-slate-800 bg-slate-900/80">
              <div className="p-3 text-xs text-slate-500 font-bold">{t('prem.feature')}</div>
              <div className="p-3 text-xs text-slate-500 font-bold text-center border-l border-slate-800">{t('prem.free')}</div>
              <div className="p-3 text-xs text-amber-400 font-bold text-center border-l border-slate-800 bg-amber-500/5">{t('prem.premium')}</div>
            </div>
            {rows.map((row, idx) => (
        ...
        {/* {t('prem.trusted_by').split(' ')[0]} yorumu */}
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4 relative">
          ...
          <p className="text-sm text-slate-300 leading-relaxed mt-1 italic">"{t('prem.testimonial')}"</p>
          <div className="flex items-center space-x-2 mt-3">
            ...
            <div>
              <p className="text-xs font-bold text-white">{t('prem.user_name')}</p>
              <p className="text-[10px] text-slate-500">{t('prem.user_desc')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          {[
            { icon: Shield, label: t('prem.badge_refund'), color: 'text-green-400' },
            { icon: Lock, label: t('prem.badge_ssl'), color: 'text-blue-400' },
            { icon: Users, label: `12.000+\n${t('prem.trusted_by').split(' ')[0]}`, color: 'text-amber-400' },
        ...
        >
          {isLoading ? (
            <div className="flex items-center space-x-2">
              ...
              <span>{t('prem.processing')}</span>
            </div>
          ) : isPremium ? (
            <><Crown size={18} className="fill-amber-400 text-amber-400" /><span>{t('prem.already_active')}</span></>
          ) : (
            <><Zap size={20} className="fill-white" /><span>{billingCycle === 'yearly' ? `₺${YEARLY_PRICE}{t('prem.per_year')}` : `₺${MONTHLY_PRICE}{t('prem.per_month')}`} ile Premium'a Geç</span></>
          )}
        </button>
        <p className="text-[10px] text-center text-slate-600 mt-2.5">{t('prem.terms')}</p>
      </div>
    </div>
  );
};
