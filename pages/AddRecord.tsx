            ...
            <div className="space-y-6">
                    ...
                    <div className="flex space-x-3 overflow-x-auto pb-3 -mx-1 px-1 hide-scrollbar">
                        {QUICK_ACTIONS.map((action) => (
                            <button
                                ...
                                className={`flex-shrink-0 px-5 py-3 rounded-full text-sm font-bold border transition-all flex items-center space-x-2 ${formData.serviceType === action.value
                        ...
                        <div className="grid grid-cols-3 gap-3 animate-fadeIn">
                            {['Credit Card', 'Cash', 'Other'].map(method => (
                                <button
                                    ...
                                    className={`text-xs py-2.5 rounded-lg font-medium border transition-colors ${formData.paymentMethod === method
                ...
                <div className="pt-4 pb-12">
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="btn-premium-3d w-full py-5 text-lg disabled:opacity-50"
                    >
                        {saving ? (
                            <div className="flex items-center gap-2">
                                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                <span>{t('add_record.saving')}</span>
                            </div>
                        ) : (
                            <>
                                <ClipboardCheck size={20} />
                                <span>{t('add_record.save')}</span>
                            </>
                        )}
                    </button>
                </div>
